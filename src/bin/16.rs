...
fn main() -> Result<()> {
    ...
    fn part2(input: &str) -> Result<i32> {
        let grid = Grid::parse(input);
        let cost_grid = &mut Grid::same_size_with(&grid, [i32::MAX; 22]);
        let mark_grid = &mut Grid::same_size_with(&grid, 0i32);
        let start = grid.find(b'S').unwrap();
        let end = grid.find(b'E').unwrap();
        let position = &mut start.clone();
        let direction = &mut RIGHT;
        ...
        let mut path = Vec::new();
        // let i = search(*position, *direction, 0, path, &grid, 0, current_cost);
        let i = search_w_lookup_and_marking(
            *position, *direction, 0, path, &grid, 0, cost_grid, mark_grid,
        );
        Ok((*mark_grid).bytes.iter().sum()) + 1
    }
