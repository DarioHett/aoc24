...
fn main() -> Result<()> {
    ...
    fn part2(input: &str) -> Result<usize> {
        ...
        // Find diffs where either horizontally or vertically 2 steps are > 100
        let mut ctr = 0;
        let dirs = (-20i32..21i32)
            .map(|x| {
                ((-20i32 + x.abs())..(21i32 - x.abs()))
                    .map(|y| Point::new(x, y))
                    .collect::<Vec<_>>()
            })
            .flatten()
            .filter(|pt| (pt.x.abs() + pt.y.abs()) > 1)
            .collect::<Vec<_>>();
        for (x, y) in (0..move_grid.width).cartesian_product(0..move_grid.height) {
            let pt = Point::new(x, y);
            if (move_grid[pt] == b'#') {
            ...
            for &dir in dirs.iter() {
                if move_grid.contains(pt + dir) {
                    if (move_grid[pt + dir] != b'#') {
                        let saving = (time_grid[pt].abs_diff(time_grid[pt + dir])
                            - (dir.x.abs() + dir.y.abs()) as usize);
                        // println!("Found: {:?}:{:?}:{:?}", pt, pt+dir, saving);
                        if saving >= 100 {
                            ctr += 1;
                        }
                    };
