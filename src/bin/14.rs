...
pub fn after_one_second(x: &isize, y: &isize, dx: &isize, dy: &isize, max_x:&isize, max_y: &isize) -> (isize, isize) {
...
fn main() -> Result<()> {
    ...
    fn part1(input: &str) -> Result<usize> {
        ...
        let res = map.iter().map(|(k,v)| {if *k > 0 {v.len()} else {1}}).product::<usize>();
        Ok(res)
    }
